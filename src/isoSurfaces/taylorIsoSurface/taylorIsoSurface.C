...
namespace Foam
{
    defineTypeNameAndDebug(taylorIsoSurface, 0);
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

bool Foam::taylorIsoSurface::noTransform(const tensor& tt) const
{
    return
...
bool Foam::taylorIsoSurface::collocatedPatch(const polyPatch& pp)
{
...
Foam::PackedBoolList Foam::taylorIsoSurface::collocatedFaces
(
...
void Foam::taylorIsoSurface::syncUnseparatedPoints
(
...
Foam::scalar Foam::taylorIsoSurface::isoFraction
(
    const scalar s0,
    const scalar s1
) const
...
bool Foam::taylorIsoSurface::isEdgeOfFaceCut
(
...
void Foam::taylorIsoSurface::getNeighbour
(
...
void Foam::taylorIsoSurface::calcCutTypes
(
    const labelList& boundaryRegion,
    const volVectorField& meshC,
    const volScalarField& cVals,
...
)
{
    ...
    if (debug)
    {
        Pout<< "taylorIsoSurface : detected " << nCutCells_
...
Foam::point Foam::taylorIsoSurface::calcCentre(const triSurface& s)
{
    vector sum = Zero;
...
void Foam::taylorIsoSurface::calcSnappedCc
(
    const labelList& boundaryRegion,
    const volVectorField& meshC,
    const volScalarField& cVals,
...
void Foam::taylorIsoSurface::calcSnappedPoint
(
...
Foam::triSurface Foam::taylorIsoSurface::stitchTriPoints
(
...
) const
{
    ...
    if (checkDuplicates)
    {
        ...
        if (debug)
        {
            Pout<< "taylorIsoSurface : merged from " << nTris
...
bool Foam::taylorIsoSurface::validTri(const triSurface& surf, const label facei)
{
...
Foam::triSurface Foam::taylorIsoSurface::subsetMesh
(
    const triSurface& s,
...
Foam::taylorIsoSurface::taylorIsoSurface
(
    const volScalarField& cVals,
    const scalarField& pVals,
    const scalar iso,
    ...
    mergeDistance_(mergeTol*mesh_.bounds().mag())
{
    ...
    if (debug)
    {
        Pout<< "taylorIsoSurface : shifted " << snappedPoints.size()
    ...
    if (debug)
    {
        Pout<< "taylorIsoSurface : shifted " << snappedPoints.size()-nCellSnaps
...

    {
        ...
        if (debug)
        {
            Pout<< "taylorIsoSurface : generated " << triMeshCells.size()
        ...
        if (debug)
        {
            Pout<< "taylorIsoSurface : generated " << triMap.size()
    ...
    if (debug)
    {
        Pout<< "taylorIsoSurface : checking " << size()

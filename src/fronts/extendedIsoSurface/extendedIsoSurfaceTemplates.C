...
<
    ...
    volMesh
> >
extendedIsoSurface::adaptPatchFields
(
...
template<class Type>
Type extendedIsoSurface::generatePoint
(
    const scalar s0,
    const Type& p0,
    const bool hasSnap0,
...
template<class Type>
void extendedIsoSurface::generateTriPoints
(
    const scalar s0,
    const Type& p0,
    const bool hasSnap0,
...
template<class Type>
label extendedIsoSurface::generateFaceTriPoints
(
...
) const
{
    ...
    {
        FatalErrorIn("extendedIsoSurface::generateTriPoints(..)")
            << "Incorrect size." << endl
...
//template<class Type>
//tmp<Field<Type> >
//extendedIsoSurface::sample(const Field<Type>& vField) const
//{
...
template<class Type>
tmp<Field<Type> >
extendedIsoSurface::interpolate
(
    const GeometricField<Type, fvPatchField, volMesh>& cCoords,
    const Field<Type>& pCoords
) const
{
    ...
    if (debug)
    {
        ...
        forAll(nValues, i)
        {
            if (nValues[i] == 0)
            {
                FatalErrorIn("extendedIsoSurface::interpolate(..)")

...
defineTypeNameAndDebug(extendedIsoSurface, 0);


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

bool extendedIsoSurface::noTransform(const tensor& tt) const
{
    return
...
bool extendedIsoSurface::collocatedPatch(const polyPatch& pp)
{
...
// Calculates per face whether couple is collocated.
PackedBoolList extendedIsoSurface::collocatedFaces
(
...
) const
{
    ...
    else
    {
        FatalErrorIn
        (
            "extendedIsoSurface::collocatedFaces(const coupledPolyPatch&) const"
...
void extendedIsoSurface::syncUnseparatedPoints
(
...
scalar extendedIsoSurface::isoFraction
(
    const scalar s0,
    const scalar s1
) const
...
bool extendedIsoSurface::isEdgeOfFaceCut
(
...
void extendedIsoSurface::getNeighbour
(
...
void extendedIsoSurface::calcCutTypes
(
    const labelList& boundaryRegion,
    const volVectorField& meshC,
    const volScalarField& cVals,
...
)
{
    ...
    if (debug)
    {
        Pout<< "extendedIsoSurface : detected " << nCutCells_
...
labelPair extendedIsoSurface::findCommonPoints
(
...
point extendedIsoSurface::calcCentre(const triSurface& s)
{
...
pointIndexHit extendedIsoSurface::collapseSurface
(
...
// to a single point
void extendedIsoSurface::calcSnappedCc
(
    const labelList& boundaryRegion,
    const volVectorField& meshC,
    const volScalarField& cVals,
...
// to a single point
void extendedIsoSurface::calcSnappedPoint
(
...
triSurface extendedIsoSurface::stitchTriPoints
(
...
) const
{
    ...
    {
        FatalErrorIn("extendedIsoSurface::stitchTriPoints(..)")
    ...
    if (checkDuplicates)
    {
        ...
        if (debug)
        {
            Pout<< "extendedIsoSurface : merged from " << nTris
...
bool extendedIsoSurface::validTri(const triSurface& surf, const label faceI)
{
...
void extendedIsoSurface::calcAddressing
(
...
) const
{
    ...
    if (debug)
    {
        Pout<< "extendedIsoSurface : detected "
            << mergedCentres.size()
...
void extendedIsoSurface::walkOrientation
(
    const triSurface& surf,
        ...
        forAll(changedFaces, i)
        {
            ...
            forAll(fEdges, fp)
            {
                ...
                if (nbrI != -1 && flipState[nbrI] == -1)
                {
                    ...
                    if (nbrFp == -1)
                    {
                        FatalErrorIn("extendedIsoSurface::walkOrientation(..)")
                            << "triI:" << triI
                            << " tri:" << tri
                            << " p0:" << p0
                            << " p1:" << p1
...
void extendedIsoSurface::orientSurface
(
    triSurface& surf,
    const List<FixedList<label, 3> >& faceEdges,
    const labelList& edgeFace0,
...
)
{
    ...
    forAll(surf, triI)
    {
        ...
        else if (flipState[triI] == -1)
        {
            FatalErrorIn
            (
                "extendedIsoSurface::orientSurface(triSurface&, const label)"
...
bool extendedIsoSurface::danglingTriangle
(
...
label extendedIsoSurface::markDanglingTriangles
(
    const List<FixedList<label, 3> >& faceEdges,
    const labelList& edgeFace0,
    const labelList& edgeFace1,
...
triSurface extendedIsoSurface::subsetMesh
(
    const triSurface& s,
...
extendedIsoSurface::extendedIsoSurface
(
    const volScalarField& cVals,
    const scalarField& pVals,
    const scalar iso,
    ...
    mergeDistance_(mergeTol*mesh_.bounds().mag())
{
    ...
    if (debug)
    {
        Pout<< "extendedIsoSurface : shifted " << snappedPoints.size()
    ...
    if (debug)
    {
        Pout<< "extendedIsoSurface : shifted " << snappedPoints.size()-nCellSnaps
    ...
    if (debug)
    {
        Pout<< "extendedIsoSurface : generated " << triMeshCells.size()
    ...
    if (debug)
    {
        Pout<< "extendedIsoSurface : generated " << triMap.size()
    ...
    if (debug)
    {
        Pout<< "extendedIsoSurface : checking " << size()
    ...
    if (false)
    {
        ...
        while (true)
        {
            ...
            if (debug)
            {
                Pout<< "extendedIsoSurface : detected " << nDangling

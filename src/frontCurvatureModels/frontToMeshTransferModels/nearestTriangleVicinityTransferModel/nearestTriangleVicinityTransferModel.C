...
void nearestTriangleVicinityTransferModel::initializeFaceCurvatureField(const fvMesh& mesh) const
{
    if (faceCurvatureTmp_.empty())
    {
...
void nearestTriangleVicinityTransferModel::resetCurvature() const
{
    auto& faceCurvature = faceCurvatureTmp_.ref();
    faceCurvature = dimensionedScalar{
                        "zero", 
                        dimless/dimLength, 
                        0.0
                    };
...
{
    initializeFaceCurvatureField(mesh);
    resetCurvature();
